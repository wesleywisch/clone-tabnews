...
describe("PATCH /api/v1/activations/[token_id]", () => {
  describe("Anonymous User", () => {
    ...
    test("With expired token", async () => {
      jest.useFakeTimers({
        now: new Date(Date.now() - activation.EXPIRAION_IN_MILLISECONDS),
      });
    ...
    test("With valid token", async () => {
      ...
      expect(expiresAt - createdAt).toBe(activation.EXPIRAION_IN_MILLISECONDS);
    ...
    test("With valid token but already activated user", async () => {
      const createdUser = await orchestrator.createUser();
      await orchestrator.activatedUser(createdUser.id);
      const activationToken = await activation.create(createdUser.id);
  ...
  describe("Default user", () => {
    test("With Valid token, but already logged in user", async () => {
      ...
      await orchestrator.activatedUser(user1.id);
